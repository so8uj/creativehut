const portfolio = document.querySelector('.portfolio-gallery'),
		...
		portfolioCats = document.querySelectorAll('.portfolio-cats > button');
	...
	function positionItems(items) {
		...
		items.forEach((item, i) => {
			item.style.cssText = `transform: translate3d(${x * (parentWidth / rowItems)}px, ${y * 365}px, 0); opacity: 1;`;
			x++;
			if (x % rowItems == 0) {
				y++;
				x = 0;
		...
		portfolio.style.height = `${Math.ceil(itemCount / rowItems) * 420}px`;
	}
